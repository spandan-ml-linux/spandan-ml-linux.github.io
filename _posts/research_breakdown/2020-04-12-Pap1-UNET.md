...
Each pixel of the segmentation map may be one of four classes(black, blue, green or red). Entropy as shown in \\( \eqref{eq1} \\) quantifies the uncertainty in a vector. It is maximum when all events are equiprobable and zero when one event is certain and the remaining are zero (In our case, the exact nature of the pixel is certain).
